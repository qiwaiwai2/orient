                ...
                <div class="table">
                    <div class="yfp"  v-show="operate_type==1">
                        <div class="yfphead">
                            ...
                            <a-button type="primary" @click="Untying">一键解绑</a-button>
                        </div>
...
<script>
import {findAllCompetition,findEntity,findEntityByGps,findAllGpsList,updateEntityAndGps,findEntityByNoGps,replaceSportsmanGps,unBindGps} from '@/api'
export default {
    data(){
        return {
            spinning:false,
    ...
    methods:{
        Untying(){//一键解绑
                this.$tips.confirm('是否确认一键解绑？').then((res) => {
                    unBindGps(this.currentGameId).then(res=>{
                        console.log(res)
                        if(res.status==1){
                            this.$message.success('解绑成功！',1).then(res=>{
                                this.selectedMan=[]
                                this.operate_type=1
