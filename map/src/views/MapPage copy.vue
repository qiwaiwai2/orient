          ...
          if (that.manList && that.manList.id.length > 0) {
            ...
            for (let i = 0; i < that.manList.id.length; i++) {
              ...
              if (that.manList["PointArr" + id]) {
                ...
                if(!that.isSync){//实时
                  let starttime = '2019-10-23 11:10:00'
                  let thistime
                  for(let i = 0;i<that.manList.item.length;i++){
                    let manitem = that.manList.item[i]
                    if(manitem.id == "8ca0f3e8b31547a99391ebfe7a6a695e"){
                      thistime = '2019-10-23 12:22:48'
                    }else if(manitem.id == "3d3781ad16ab4257942a245621e8f1e6"){
                      thistime = '2019-10-23 11:48:49'
                    }else{
                      thistime = '2019-10-23 11:10:00'
                    }
                  }
                  setTimeout(() => {
                    that.manList["marker" + id].moveAlong(
              ...
              if (item["compare" + data.id].length <= 1) {
                if (!item.marker) {
                  ...
                  item.marker = new that.theamap.Marker({
                    ...
                    content:
                      '<div style="display:inline-block;width:14px;height:14px;border-radius:50%;background:' + item.color +';border:2px solid #fff"></div>',
