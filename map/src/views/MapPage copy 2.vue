...
export default {
  ...
  methods: {
              ...
              if (data.id == item.id) {
                ...
                if (item["compare" + data.id].length <= 1) {
                  if (!item.marker) {
                    ...
                    //第一次拿到选手数据
                    that.createmap.setCenter([data.crossGpsData.longitude,data.crossGpsData.latitude])
            ...
            for (let i = 0; i < that.manList.item.length; i++) {
              ...
              if (that.manList["PointArr" + id]) {
                ...
                if(!that.isSync){ //实时
                  ...
                  console.log(that.getTimeDiff(thistime,starttime)/((that.speedNumber)))
                  console.log(that.manList["speed" + id])
                  setTimeout(() => {
                    that.manList["historymarker" + id].moveAlong (
                      listArr,
                      that.manList["speed" + id] * that.speedNumber
    ...
    startTimer() {
      //开始计时器
      this.inter = setInterval( this.timer, 1000/((this.speedNumber/2)+10) )
    },
    stopTimer() {
      //停止计时器
      clearInterval(this.inter)
        ...
        if (res.status == 1) {
          ...
          for (var i = 0; i < man.item.length; i++) {
            ...
            man["marker" + item] = new that.theamap.Marker({
              ...
              position: man["arr" + item][0],
              content:
                '<div style="display:inline-block;width:'+that.MarkerSize+'px;height:'+that.MarkerSize+'px;border-radius:50%;background-color:'+that.colorArr[theindex+1]+';border:2px solid #fff"></div>',
              offset: new that.theamap.Pixel(-7, -7),
              label: {
                offset: new that.theamap.Pixel(5, 5), //设置文本标注偏移量
                direction: "right",
  ...
  watch:{
    inputValue(val) {
      let time = '2019-10-20 10:00:00'
      let datetime = new Date(time)
      let newdate = new Date(datetime.setSeconds(datetime.getSeconds()+val))
