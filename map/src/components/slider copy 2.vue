            ...
            <ul class="player_list">
                <li v-for="(item,index) in playInfo" :key="index" :class="item.active?'player_active':''"  @click="selectPlay(index)">
                    <div class="player_info">
                        <p>
                            <span :style="{backgroundColor:item.isSelect?item.color:'#fff'}" @click.stop="selectPlayer(index)"></span>
                            <span>{{item.name}}</span>
                        </p>
                        <p>
                            <!-- <span>11:23:00</span>
    ...
    methods:{
        ...
        selectAll(){//选择全部
            ...
            this.playInfo.forEach((item,itemIndex)=>{
                if(this.isSelectAll){
                    ...
                    if(this.endTimeList&&this.endTimeList.length>0){
                        let endTime
                        for(let i = 0;i<this.endTimeList.length;i++){
                            let endTimeItem = this.endTimeList[i]
                            if(endTimeItem.cardno == item.fingerCardNo ){
                                if(endTimeItem.endTime){
                                    endTime = item.startTime.substring(0,10) +" "+ endTimeItem.endTime
                                }else{
                                    endTime = item.startTime.substring(0,10) +" "+ '16:00:00'
                                }
                                
                            }
                        }
                        this.selectManid.push({
                            id:item.id,
                        ...
                        this.selectManid.push({
                            id:item.id,
                            startTime:item.startTime,
                            endTime:'2019-10-23 17:00:00'
                        })
                    }
                    console.log(this.selectManid)
                }else{
            ...
            this.$emit('selectedManid',this.selectManid)//将值绑定到transfer上传递过去
        },
        ...
        selectPlayer(index){//选择选手
            if(this.playInfo[index].isSelect){
                ...
                if(this.selectManid.indexOf(manid)>=0){
                    this.selectManid.splice(this.selectManid.indexOf(manid),1)
                }
                for(let i = 0;i<this.selectManid.length;i++){
                    let item = this.selectManid[i]
                    if(item.id == manid){
                        this.selectManid.splice(i,1)
                    }
                }
                // console.log(this.selectManid)
            }else{
                ...
                if(this.endTimeList&&this.endTimeList.length>0){
                    ...
                    for(let i = 0;i<this.endTimeList.length;i++){
                        let endTimeItem = this.endTimeList[i]
                        if(endTimeItem.cardno == this.playInfo[index].fingerCardNo ){
                            if(endTimeItem.endTime){
                                endTime =  this.playInfo[index].startTime.substring(0,10) + " " + endTimeItem.endTime
                            }else{
                                endTime =  this.playInfo[index].startTime.substring(0,10) + " " + '16:00:00'
                            }
                        }
                    }
                    // this.selectManid.push(this.playInfo[index].id)
                    this.selectManid.push({
                        id:this.playInfo[index].id,
                        startTime:this.playInfo[index].startTime,
                        endTime:endTime
                    })
                }else{
                    // this.selectManid.push(this.playInfo[index].id)
            ...
            this.$emit('selectedManid',this.selectManid)
        },
